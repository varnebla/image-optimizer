  ...
  <div v-if="results.length > 0" class="my-8">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
      <div class="p-4 bg-blue-50 rounded-lg border border-blue-200">
        <p class="text-sm text-blue-700 mb-1">{{ t('detailedStats.totalProcessed') }}</p>
        <p class="text-2xl font-bold text-blue-900">{{ results.length }}</p>
      </div>
      <div class="p-4 bg-purple-50 rounded-lg border border-purple-200">
        <p class="text-sm text-purple-700 mb-1">{{ t('detailedStats.totalTime') }}</p>
        <p class="text-2xl font-bold text-purple-900">{{ totalTime }}s</p>
      </div>
      <div class="p-4 bg-green-50 rounded-lg border border-green-200">
        <p class="text-sm text-green-700 mb-1">{{ t('detailedStats.avgSavings') }}</p>
        <p class="text-2xl font-bold text-green-900">{{ avgSavings }}%</p>
      </div>
      <div class="p-4 bg-orange-50 rounded-lg border border-orange-200">
        <p class="text-sm text-orange-700 mb-1">{{ t('detailedStats.avgSize') }}</p>
        ...
        <table class="w-full text-sm">
          <thead class="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              >
                {{ t('detailedStats.file') }} {{ getSortIcon('originalName') }}
              </th>
              <th
                class="px-4 py-3 text-right font-semibold text-gray-700 cursor-pointer hover:bg-gray-100"
                @click="sortBy('originalSize')"
              >
                {{ t('detailedStats.original') }} {{ getSortIcon('originalSize') }}
              </th>
              <th
                class="px-4 py-3 text-right font-semibold text-gray-700 cursor-pointer hover:bg-gray-100"
                @click="sortBy('optimizedSize')"
              >
                {{ t('detailedStats.optimized') }} {{ getSortIcon('optimizedSize') }}
              </th>
              <th
                class="px-4 py-3 text-right font-semibold text-gray-700 cursor-pointer hover:bg-gray-100"
                @click="sortBy('savings')"
              >
                {{ t('detailedStats.savings') }} {{ getSortIcon('savings') }}
              </th>
              <th class="px-4 py-3 text-center font-semibold text-gray-700">
                {{ t('detailedStats.dimensions') }}
              </th>
              <th class="px-4 py-3 text-center font-semibold text-gray-700">
                {{ t('detailedStats.format') }}
              </th>
              <th
                class="px-4 py-3 text-right font-semibold text-gray-700 cursor-pointer hover:bg-gray-100"
                @click="sortBy('processingTime')"
              >
                {{ t('detailedStats.time') }} {{ getSortIcon('processingTime') }}
              </th>
              <th class="px-4 py-3 text-center font-semibold text-gray-700">
                {{ t('detailedStats.status') }}
              </th>
            </tr>
          </thead>
          <tbody>
...
function exportToCSV() {
  // Crear CSV
  const headers = [
    t('detailedStats.csvOriginalFile'),
    t('detailedStats.csvOriginalSize'),
    t('detailedStats.csvOptimizedSize'),
    t('detailedStats.csvSavingsPercent'),
    t('detailedStats.csvOriginalWidth'),
    t('detailedStats.csvOriginalHeight'),
    t('detailedStats.csvFinalWidth'),
    t('detailedStats.csvFinalHeight'),
    t('detailedStats.csvFormat'),
    t('detailedStats.csvTime'),
  ];
